...
const UserDetail = () => {
  ...
  const [activeTab, setActiveTab] = useState<"video" | "repost" | "like">("video");
  ...
  return (
    <div className="max-w-9xl mx-auto p-10 bg-white/90 border rounded-2xl shadow-2xl hover:shadow-3xl transition-shadow duration-300 h-[90vh] flex flex-col">
      ...
      <div className="flex-1 flex items-center space-x-6 mb-6">
        ...
        <div>
          <h1 className="text-4xl font-bold text-gray-800">{userData.fullname}</h1>
          ...
          <p className="text-lg text-gray-600 flex items-center space-x-2">
            <FaRegEnvelope className="text-gray-500" /> <span>{userData.email}</span>
          </p>
          <div className="flex space-x-4 mt-3">
            <Button icon={<FiUserPlus />} text="Follow" className="bg-blue-500 hover:bg-blue-600 text-white" />
            <Button icon={<FiMessageSquare />} text="Message" className="bg-gray-200 hover:bg-gray-300 text-gray-700" />
            <Button icon={<FiShare2 />} text="Share" className="bg-gray-200 hover:bg-gray-300 text-gray-700" />
            <Button icon={<FaEllipsisH />} className="bg-gray-200 hover:bg-gray-300 text-gray-700" />
          </div>
        </div>
      </div>
          ...
          {["video", "repost", "like"].map((tab) => (
            <button
              ...
              className={`text-lg font-semibold p-2 flex-1 text-center ${
                activeTab === tab ? "border-b-4 border-blue-500 text-blue-600" : "text-gray-600"
              }`}
...
const Button = ({ icon, text, className }: { icon: JSX.Element; text?: string; className: string }) => (
  <button className={`px-5 py-3 rounded-lg flex items-center space-x-2 shadow-md transition-all duration-300 ${className}`}>
